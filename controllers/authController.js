...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2) Check if the user exist && if password is correct
...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Get token and check if there is a token
  let token;
  if (
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Generate the ran dom reset token
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) if token has not expired && there is a user, then set new password
  if (!user) {
